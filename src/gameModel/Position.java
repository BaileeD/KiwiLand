...
/**
 * Represents the location of any item/grid square/player in the game. Positions
 * must always be valid for the particular game. Valid positions include
 * somewhere on the island or surrounding sea, or the NotOnIsland position which
 * is used for such things as in the player's backpack.
 *
 * @author AS
...
public class Position
{
	...
	/**
	 * Position for objects that are not on the island, e.g., in the player's
	 * backpack.
	 */
	public static final Position NOT_ON_ISLAND = new Position();

	/**
	 * Constructs a position that is not on the island (e.g., player's
	 * backpack).
	 */
	private Position()
	{
	...
	/**
	 * Constructor for objects of class Position Can only construct valid
	 * positions. Invalid arguments result in exceptions.
	 *
	...
	public Position(Island island, int row, int column)
	{
		if (island == null)
		{
			throw new IllegalArgumentException("Island parameter cannot be null");
		}
	...
	/**
	 * Creates a valid new position if a move in this direction is valid from
	 * the current position. It is not possible to move outside of the island.
	...
	public Position getNewPosition(MoveDirection direction)
	{
		...
		if (isOnIsland())
		{
			...
			else if ((direction == MoveDirection.EAST) && (column < island.getNumColumns() - 1))
			{
				newPosition = new Position(island, row, column + 1);
			}
			else if ((direction == MoveDirection.SOUTH) && (row < island.getNumRows() - 1))
			{
				newPosition = new Position(island, row + 1, column);
			}
